...
SDL_Surface              *gpScreenScale2x           = NULL;
...
#if (defined (__SYMBIAN32__) && !defined (__S60_5X__)) || defined (PSP) || defined (ANDROID)
...
#ifdef WVGA
   gpScreenScale2x = SDL_CreateRGBSurface(gpScreenReal->flags & ~SDL_HWSURFACE, 640, 400, 8,
      gpScreenReal->format->Rmask, gpScreenReal->format->Gmask,
      gpScreenReal->format->Bmask, gpScreenReal->format->Amask);
#endif
...
VOID
VIDEO_GetFitRect(const SDL_Rect  *srcRect,
		...
		SDL_Rect* newRect)
{
	...
	if (d1 < d2) {
			...
			newRect->w = w * d1;
			newRect->h = h*d1;
			newRect->y = (dstRect->h - newRect->h)/2;
	}
	...
	else
	{
			...
			newRect->w = w * d2;
			newRect->h = h*d2;
			newRect->x = (dstRect->w - newRect->w)/2;
	}
}
VOID
VIDEO_UpdateScreen(
...
--*/
{
   static SDL_Rect        srcrect, uptrect;
   static SDL_Rect		  gsrect, gdrect,fitscreenRect;
   static short           screenRealWidth = 0;
   static short           screenRealHeight = 0;
   static short           screenRealX = 0;
   static short           screenRealY = 0;
   ...
   if (SDL_MUSTLOCK(gpScreenReal))
   {
	...
	gdrect.x = 0;
	gdrect.y = 0;
	gdrect.w = gpScreenReal->w;
	gdrect.h = gpScreenReal->h;
   if (!bScaleScreen)
   {

   }
   VIDEO_GetFitRect(&gsrect, &gdrect, &fitscreenRect);
   screenRealWidth = fitscreenRect.w;
   screenRealHeight = fitscreenRect.h;
   screenRealX = fitscreenRect.x;
   screenRealY = fitscreenRect.y;
   if (lpRect != NULL)
   {

	   uptrect.x = (WORD)((DWORD)(lpRect->x) * screenRealWidth / gpScreen->w + screenRealX);
	   uptrect.y = (WORD)((DWORD)(lpRect->y) * screenRealHeight / gpScreen->h + screenRealY);
	   uptrect.w = (WORD)((DWORD)(lpRect->w) * screenRealWidth / gpScreen->w);
	   uptrect.h = (WORD)((DWORD)(lpRect->h) * screenRealHeight / gpScreen->h);

      SDL_SoftStretch(gpScreen, (SDL_Rect *)lpRect, gpScreenReal, &uptrect);
      ...
      SDL_UpdateRect(gpScreenReal, uptrect.x, uptrect.y, uptrect.w, uptrect.h);
   }
   else if (g_wShakeTime != 0)
   {
      ...
      {
    	  fitscreenRect.y = (screenRealY + g_wShakeLevel) * screenRealHeight / gpScreen->h;
      }

      SDL_SoftStretch(gpScreen, &srcrect, gpScreenReal, &fitscreenRect);

      if (g_wShakeTime & 1)
      {
    	  fitscreenRect.y = (screenRealY + screenRealHeight - g_wShakeLevel) * screenRealHeight / gpScreen->h;
      }
      else
      {
    	  fitscreenRect.y = screenRealY;
      }

      fitscreenRect.h = g_wShakeLevel * screenRealHeight / gpScreen->h;

      SDL_FillRect(gpScreenReal, &fitscreenRect, 0);
   ...
   else
   {
      ...
      SDL_SoftStretch(gpScreen, NULL, gpScreenReal, &fitscreenRect);
